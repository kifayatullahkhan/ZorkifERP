...
$query_RS_SaleStock = "SELECT erp_werehouses.WHName , erp_inventoryitems.InvItemDescription ,  erp_inventoryitemstypes.InvItemTypeName ,  erp_inventoryitemsunits.InvItemUnitName , erp_customerorderdetails.Price_Per_Unit , erp_inventoryitems.StockPositionQuantity , SUM(erp_customerorderdetails.Quantity) AS QuantitySale    FROM erp_inventoryitems JOIN  erp_inventoryitemsunits  ON  erp_inventoryitemsunits.InvItemUnitID = erp_inventoryitems.InvItemUnitID  JOIN erp_inventoryitemstypes  ON  erp_inventoryitemstypes.InvItemTypeID =  erp_inventoryitems.InvItemTypeID  JOIN erp_customerorderdetails  ON  erp_customerorderdetails.InvItemID = erp_inventoryitems.InvItemID  JOIN erp_customerorders  ON  erp_customerorders.CustomerOrderID = erp_customerorderdetails.CustomerOrderID  JOIN  erp_werehouses  ON  erp_werehouses.WHID = erp_inventoryitems.WHID  WHERE erp_customerorders.OrderDate BETWEEN '$from' AND '$to' AND erp_inventoryitems.InvItemID = erp_customerorderdetails.InvItemID  GROUP BY erp_inventoryitems.InvItemID  ORDER BY erp_inventoryitems.InvItemID";
...
<h2>Inventory Items Stock Postion</h2>
  <form id="form1" name="form1" method="post" action="">
      ...
      <tr>
        ...
        <td><strong><?php echo $row_RS_SaleStock['WHName']; ?></strong></td>
      </tr>
      <tr>
